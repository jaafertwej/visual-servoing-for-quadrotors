...
class Camera():
    ...
    def __init__(self, d_image, depth):
        ...
        self.K = np.array([[476.7030836014194, 0.0              , 400.5],
                           ...
                           [0.0              , 0.0              , 1.0  ]])
# P: [476.7030836014194, 0.0, 400.5, -33.36921585209936,
    ...
    def image_callback(self, msg):
        try:
            ...
            if len(self.result) != 0:
                ...
                self.ee.append(eh)
                # k_f_x = adaptive_gain(5, 1, 20, np.linalg.norm(eh[:2], np.inf))
                # k_f_z = adaptive_gain(7, .8, 20, np.linalg.norm(eh, np.inf))
                # k_f = np.diagflat([[k_f_x, k_f_x], [k_f_z, 1.5]])
                k_f = 1.4
...
def main():
    ...
    d_image = ndimage.imread('../features_uav_omni.png')
    ...
    plt.ylim([-2, 2])
    for i in range(4):
        plt.plot(t1, np.array(cam.ee)[:, i], color = color[i], label= label2[i])
